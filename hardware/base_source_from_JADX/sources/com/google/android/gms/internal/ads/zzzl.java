package com.google.android.gms.internal.ads;

import android.os.Bundle;
import com.google.android.gms.dynamic.IObjectWrapper;

public final class zzzl extends zzxm {
    /* access modifiers changed from: private */
    public zzxa zzbnn;

    public final void destroy() {
    }

    public final String getAdUnitId() {
        return null;
    }

    public final String getMediationAdapterClassName() {
        return null;
    }

    public final zzyp getVideoController() {
        return null;
    }

    public final boolean isLoading() {
        return false;
    }

    public final boolean isReady() {
        return false;
    }

    public final void pause() {
    }

    public final void resume() {
    }

    public final void setImmersiveMode(boolean z) {
    }

    public final void setManualImpressionsEnabled(boolean z) {
    }

    public final void setUserId(String str) {
    }

    public final void showInterstitial() {
    }

    public final void stopLoading() {
    }

    public final void zza(zzabg zzabg) {
    }

    public final void zza(zzaow zzaow) {
    }

    public final void zza(zzapc zzapc, String str) {
    }

    public final void zza(zzavb zzavb) {
    }

    public final void zza(zzwf zzwf) {
    }

    public final void zza(zzwx zzwx) {
    }

    public final void zza(zzxq zzxq) {
    }

    public final void zza(zzxt zzxt) {
    }

    public final void zza(zzxz zzxz) {
    }

    public final void zza(zzyv zzyv) {
    }

    public final void zza(zzzw zzzw) {
    }

    public final void zzap(String str) {
    }

    public final IObjectWrapper zzie() {
        return null;
    }

    public final zzwf zzif() {
        return null;
    }

    public final void zzih() {
    }

    public final zzxt zzir() {
        return null;
    }

    public final zzxa zzis() {
        return null;
    }

    public final String zzje() {
        return null;
    }

    public final boolean zzb(zzwb zzwb) {
        zzbbd.m31e("This app is using a lightweight version of the Google Mobile Ads SDK that requires the latest Google Play services to be installed, but Google Play services is either missing or out of date.");
        zzbat.zztu.post(new zzzm(this));
        return false;
    }

    public final void zza(zzxa zzxa) {
        this.zzbnn = zzxa;
    }

    public final Bundle getAdMetadata() {
        return new Bundle();
    }
}
