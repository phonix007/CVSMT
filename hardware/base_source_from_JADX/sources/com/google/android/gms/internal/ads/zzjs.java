package com.google.android.gms.internal.ads;

public final class zzjs {

    /* renamed from: id */
    public final int f48id;
    public final int type;
    public final zzfs zzaad;
    public final long zzaan;
    public final int zzakx;
    public final long zzasl;
    public final int zzasm;
    public final zzjt[] zzasn;
    public final long[] zzaso;
    public final long[] zzasp;
    public final long zzcr;

    public zzjs(int i, int i2, long j, long j2, long j3, zzfs zzfs, int i3, zzjt[] zzjtArr, int i4, long[] jArr, long[] jArr2) {
        this.f48id = i;
        this.type = i2;
        this.zzcr = j;
        this.zzasl = j2;
        this.zzaan = j3;
        this.zzaad = zzfs;
        this.zzasm = i3;
        this.zzasn = zzjtArr;
        this.zzakx = i4;
        this.zzaso = jArr;
        this.zzasp = jArr2;
    }
}
